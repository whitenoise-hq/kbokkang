 ...
 *   레퍼런스 토스트처럼 **배경은 강조색 10%, 테두리는 강조색 100%** 로 층을 만든다.
 ...
 * - **적중이 없으면 흰 배경 + `borderStrong` 테두리**(카드처럼). 실패를 빨강으로 강조하면
 *   앱을 열 때마다 기분이 나빠진다. 회색 틴트도 써봤지만 **화면 배경(`surface`)과 겹쳐
 *   테두리로만 구분돼 존재감이 약했다.** 흰색이면 회색 화면 위에 떠 보인다.
 *
...
const styles = StyleSheet.create({
  ...
  hit: {
    backgroundColor: `${COLORS.success}${BACKGROUND_ALPHA}`,
