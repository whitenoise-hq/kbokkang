 ...
 *
 * ⚠️⚠️ **`secondary` 를 회색 화면(`surface`) 위에 쓰지 말 것.** 보조 버튼 배경이 `surface`
 *    라서 **화면과 같은 색이 되어 버튼으로 보이지 않는다**(도감 상세의 판매 버튼이 그랬다).
 *    `secondary` 는 **흰 카드 안**이나 **어두운 배경**(뽑기 개봉 화면) 전용이다.
 *    회색 화면 위의 동작은 `primary` 를 쓴다.
 *
 * ## 크기
 *
 * | size | 높이 | 쓰는 곳 |
